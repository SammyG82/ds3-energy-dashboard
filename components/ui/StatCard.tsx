...
export default function StatCard({ label, value, sub, accent = "blue", size = "2xl", isDark = false, nested = true }: StatCardProps) {
  return (
    <div className={`rounded-xl p-4 border transition-colors ${isDark ? `${nested ? "bg-white/10" : "bg-white/5"} border-white/10 hover:border-white/30` : "bg-white border-slate-200 hover:border-slate-300"}`}>
      <p className={`text-xs font-medium uppercase tracking-wider mb-1 ${isDark ? "text-white/60" : "text-slate-500"}`}>{label}</p>
